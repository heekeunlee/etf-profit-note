                ...
                <div className="px-6 mb-8 mt-4">
                    <button
                        onClick={() => {
                            ...
                            const userName = activeUser === 'heekeun' ? 'Heekeun Lee' : 'Geonkyung Lee';
                            const now = new Date().toLocaleString();
                            let report = `📊 [ETF Profit Report]\n\n`;
                            report += `👤 Investor: ${userName}\n`;
                            report += `🕒 Generated: ${now}\n\n`;
                            report += `💰 Total Profit: ₩${data.total_profit.toLocaleString()}\n`;
                            report += `🏦 Total Equity: ₩${data.total_equity.toLocaleString()}\n\n`;
                            report += `━━━━━━━━━━━━━━━━━━━\n\n`;

                            data.records.forEach(record => {
                                report += `📅 ${record.date}  |  +₩${record.daily_profit.toLocaleString()} (${record.daily_roi}%)\n`;
                                record.trades.forEach(trade => {
                                    report += `   • ${trade.name}: +₩${trade.profit.toLocaleString()} (${trade.roi}%)\n`;
                                });
                                report += `\n`;
                            });
